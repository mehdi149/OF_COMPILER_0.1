...
public class PmooAnalysis extends Analysis {	
	...
	protected static ServiceCurve computePartialPMOOServiceCurve(	Path path,
															...
															int[] server_rl_iters )
	{
		Num T = Num.createZero();
		Num R = Num.createPositiveInfinity();
		Num sum_bursts = Num.createZero();
		Num sum_latencyterms = Num.createZero();
		...
		for ( Flow f : cross_flow_substitutes )
		{
 		...
 		if( T == Num.getPositiveInfinity() ) {
 			return ServiceCurve.createNullService();
 		}
 		if( R == Num.getPositiveInfinity() ) {
