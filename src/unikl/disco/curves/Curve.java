...
public class Curve {
	...
	protected void initializeNullCurve() {
		initializeHorizontal( Num.createZero() );
	}
	...
	protected void initializeHorizontal( double y ) {
		initializeHorizontal( Num.createNum( y ) );
	}
	...
	protected void initializeZeroDelayInfiniteBurst() {
		initializeDelayedInfiniteBurst( Num.createZero() );
	}
	...
	protected void initializeDelayedInfiniteBurst( Num latency ) {
		...
		getSegment( 0 ).x        = Num.createZero();
		getSegment( 0 ).y        = Num.createZero();
		getSegment( 0 ).grad     = Num.createZero();
		getSegment( 0 ).leftopen = false;
		
		getSegment( 1 ).x        = Num.max( Num.getZero(), latency );
		getSegment( 1 ).y        = Num.createPositiveInfinity();
		getSegment( 1 ).grad     = Num.createZero();
	...
	protected void initializeTokenBucket( double rate, double burst ) {
		initializeTokenBucket( Num.createNum( rate ), Num.createNum( burst ) ) ;
	}
	...
	protected void initializeRateLatency( double rate, double latency ) {
		initializeRateLatency( Num.createNum( rate ), Num.createNum( latency ) );
	}
	...
	private static Curve createNullCurve() {
		return createHorizontal( Num.createZero() );
	}
	...
	private static Curve createTokenBucket( Num rate, Num burst ) {
		...
		c.getSegment(0).x        = Num.createZero();
		c.getSegment(0).y        = Num.createZero();
		c.getSegment(0).grad     = Num.createZero();
		c.getSegment(0).leftopen = false;
		
		c.getSegment(1).x        = Num.createZero();
	...
	public static Curve createTokenBucket( double rate, double burst ) {
		return createTokenBucket( Num.createNum( rate ), Num.createNum( burst ) ) ;
	}
	...
	private static Curve createRateLatency( Num rate, Num latency ) {
		...
		if ( latency.equals( Num.getZero() ) ) {
			...
			c.getSegment(1).y        = Num.createZero();
			c.getSegment(1).grad     = rate;
	...
	public static Curve createFromTokenBuckets(List<Curve> token_buckets) {
		Curve c = createDelayedInfiniteBurst( Num.createZero() );
	...
	private void decomposeIntoTokenBuckets() {
		...
		for (int i = 0; i < segments.length; i++) {
			...
			Num burst = Num.sub( segments[i].y, Num.mult( segments[i].x, segments[i].grad ) );
	...
	private void decomposeIntoRateLatencies() {
		...
		if( CalculatorConfig.SERVICE_CURVE_CHECKS && !this.isConvex() ) {
			if ( this.equals( ServiceCurve.createZeroDelayInfiniteBurst() ) ) {
				...
				rate_latencies.add( createRateLatency( Num.createPositiveInfinity(), Num.createZero() ) );
			} else {
			...
			for (int i = 0; i < segments.length; i++) {
				...
				Num rate = segments[i].grad;
				Num latency = Num.sub( segments[i].x, Num.div( segments[i].y, segments[i].grad ) );
	...
	public Num getHighestRate() {
		Num max_rate = Num.getNegativeInfinity(); // No need to create an object here as this negative infinity is only used for initial comparison
		for (int i = 0; i < segments.length; i++) {
	...
	public boolean isDiscontinuity( int i ) {
		return (i+1 < segments.length && ( Num.abs( Num.sub( segments[i+1].x, segments[i].x ) ) ).less( Num.getEpsilon() ) );
	}
	...
	public boolean isRealDiscontinuity( int i ) {
		return (isDiscontinuity(i) && ( Num.abs( Num.sub( segments[i+1].y, segments[i].y ) ) ).ge( Num.getEpsilon() ) );
	}
	...
	public boolean isUnrealDiscontinuity( int i ) {
		return (isDiscontinuity(i) && ( Num.abs( Num.sub(segments[i+1].y, segments[i].y ) ) ).less( Num.getEpsilon() ) );
	}
	...
	public boolean isBurstDelay() {
		return (segments.length == 2
				&& segments[0].x.equals( Num.getZero() )
				&& segments[0].y.equals( Num.getZero() )
				&& segments[0].grad.equals( Num.getZero() )
				&& !segments[0].leftopen
				&& segments[1].x.ge( Num.getZero() )
				&& segments[1].y.equals( Num.getPositiveInfinity() )
				&& segments[1].grad.equals( Num.getZero() )
				&& segments[1].leftopen);
	}
	...
	public void beautify() {
		...
		while(i < segments.length-1) {
			...
			if ( Num.abs( firstArg ).less( Num.getEpsilon() )
					&& Num.abs( secondArg ).less( Num.getEpsilon() ) ){
		...
		for (i = 0; i < segments.length-1; i++) {
			if (segments[i].x.equals( segments[i+1].x ) ) {
				segments[i].grad = Num.createZero();
			}
		}
		
		// Remove rate of tb arrival curves' first segment
		if ( segments.length > 1 
				&& segments[0].x == Num.getZero() 
				&& segments[0].y != Num.getZero()
				&& segments[1].x == Num.getZero() 
				&& segments[1].y != Num.getZero() ) {
			segments[0].grad = Num.createZero();
		}
		clearMetaInfo();
	}
	...
	public int getSegmentFirstAtValue( Num y ) {
		...
		for (int i = 0; i < segments.length; i++) {
			if (i < segments.length-1) {
				...
				if ( segments[i].grad.greater( Num.getZero() ) ) {
					return i;
				}
			}
		}
	...
	public Num f_inv( Num y, boolean rightmost) {
		...
		if (rightmost) {
			while(i < segments.length && segments[i].grad.equals( Num.getZero() ) ) {
				i++;
			}
		...
		if ( !segments[i].grad.equals( Num.getZero() ) ) {
	...
	public Num getMaxY( Num a, Num b ) {
		...
		Num ret = Num.getZero(); // No need to create an object as this value is only set for initial comparison in the loop.
		for(int i=sa; i<=sb; ++i) {
	...
	public boolean isWideSenseIncreasing() {
		Num y = Num.getNegativeInfinity(); // No need to create an object as this value is only set for initial comparison in the loop.
		for (int i = 0; i < segments.length; i++) {
			if ( segments[i].y.less( y ) || segments[i].grad.less( Num.getZero() ) ) {
				return false;
			}
			y = segments[i].y;
		}
	...
	public boolean isConvexIn( Num a, Num b ) {
		Num last_gradient = Num.getNegativeInfinity();  // No need to create an object as this value is only set for initial comparison in the loop.
	...
	public boolean isConcaveIn( Num a, Num b ) {
		Num last_gradient = Num.getPositiveInfinity(); // No need to create an object as this value is only set for initial comparison in the loop.
	...
	public boolean isAlmostConcave() {
		...
		for (int i = 0; i < segments.length; i++) {
			...
			if ( last_gradient.equals( Num.getPositiveInfinity() ) && segments[i].grad.equals( Num.getZero() ) ) {
				continue;
			}
	...
	public static Curve shiftRight( Curve curve, Num dx ) {
		...
		if ( !( ( curve_copy.getSegment(0).y ).equals( Num.getZero() ) ) ) {
	...
	public static Curve removeLatency( Curve curve ) {
		...
		while(result.segments.length > 0) {
			if ( result.segments[0].y.greater( Num.getZero() ) || result.segments[0].grad.greater( Num.getZero() ) ) {
				break;
			}
			if ( curve.getSegment(0).y.less( Num.getZero() ) || curve.getSegment(0).grad.less( Num.getZero() ) ) {
	...
	public Num getLatency() {
		if ( segments[0].y.greater( Num.getZero() ) ) {
			return Num.createZero();
		}
		for (int i = 0; i < segments.length; i++) {
			Num y0 = segments[i].y;
			if (y0.less( Num.getZero() ) && y0.greater( Num.negate( Num.getEpsilon() ) ) ) {
				y0 = Num.createZero();
			}
			if ( y0.greater( Num.getZero() )
					|| ( y0.ge( Num.getZero() ) && segments[i].grad.greater( Num.getZero() ) )
				) {
				return segments[i].x;
			}
			if (y0.less( Num.getZero() ) || segments[i].grad.less( Num.getZero() ) ) {
				throw new RuntimeException("Should have avoided neg. gradients elsewhere...");
			}
		}
	...
	public static Num getXIntersection( Curve curve1, Curve curve2 ){
		Num x_int = Num.getPositiveInfinity(); // No need to create an object as this value is only set for initial comparison in the loop.
		...
		for( int i = 0; i < curve1.getSegmentCount(); i++ ){
			...
			for( int j = 0; j < curve2.getSegmentCount(); j++ ){
				...
				if( x_int_tmp.equals( Num.getNaN() ) ) {
					break;
				}
				
				if( x_int_tmp.greater( Num.getZero() ) ){
					if( !curve1_last ){
						if( !curve2_last ){
	...
	private static Curve computeResultingCurve( Curve curve1, Curve curve2, int operator ) {
		Curve ZERO_DELAY_INFINITE_BURST = createDelayedInfiniteBurst( Num.createZero() );
		...
		while( i1 < curve1.getSegmentCount() || i2 < curve2.getSegmentCount() ) {
			Num x_next1 = ( i1+1 < curve1.getSegmentCount() ) ?
								curve1.getSegment( i1+1 ).x : Num.createPositiveInfinity();
			Num x_next2 = ( i2+1 < curve2.getSegmentCount() ) ?
								curve2.getSegment( i2+1 ).x : Num.createPositiveInfinity();
			...
			case OPERATOR_MIN:
				...
				if ( x_cross.equals( Num.getNaN() ) ) {
					x_cross = Num.createPositiveInfinity();
				}
	...
	public static Curve boundAtXAxis( Curve curve ) {
		...
		LinearSegment s;
		for ( int i = 0; i < curve_copy.getSegmentCount(); i++ ) {
			if ( curve_copy.getSegment( i ).y.greater( Num.getZero() ) ) {
				result.addSegment( curve_copy.getSegment( i ) );

				if ( curve_copy.getSegment(i).grad.less( Num.getZero() ) ) {
				...
				if ( curve_copy.getSegment(i).grad.greater( Num.getZero() ) ) {
					Num x_cross = curve_copy.getSegment( i ).getXIntersectionWith( LinearSegment.getXAxis() );
					if ( i+1 >= curve_copy.getSegmentCount() || x_cross.less( curve_copy.getSegment(i+1).x ) ) {
						s = LinearSegment.createHorizontalLine( 0.0 );
						s.x   = x_cross;
	...
	public static Num getMaxVerticalDeviation( Curve c1, Curve c2 ) {
		...
		Num burst_c1 = c1.fLimitRight( Num.getZero() );
		Num burst_c2 = c2.fLimitRight( Num.getZero() );
	...
	public static Num getMaxHorizontalDeviation( Curve c1, Curve c2 ) {
		...
		Num result = Num.createNegativeInfinity();
	...
	public static ArrayList<Num> computeInflectionPointsX( Curve c1, Curve c2 ) {
		...
		while( i1 < c1.getSegmentCount() || i2 < c2.getSegmentCount() ) {
			Num x1 = ( i1 < c1.getSegmentCount() ) ?
						c1.getSegment( i1 ).x : Num.createPositiveInfinity();
			Num x2 = ( i2 < c2.getSegmentCount() ) ?
						c2.getSegment( i2 ).x : Num.createPositiveInfinity();
			if ( x1.less( x2 ) ) {
				xcoords.add( x1.copy() );
				i1++;
	...
	public static ArrayList<Num> computeInflectionPointsY( Curve c1, Curve c2 ) {
		...
		while( i1 < c1.getSegmentCount() || i2 < c2.getSegmentCount() ) {
			Num y1 = ( i1 < c1.getSegmentCount() ) ?
						c1.getSegment( i1 ).y : Num.createPositiveInfinity();
			Num y2 = ( i2 < c2.getSegmentCount() ) ?
						c2.getSegment( i2 ).y : Num.createPositiveInfinity();
			if ( y1.less( y2 ) ) {
				ycoords.add( y1.copy() );
				i1++;
