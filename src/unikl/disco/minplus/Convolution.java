...
public class Convolution {
	...
	public static MaxServiceCurve convolve( MaxServiceCurve max_service_curve_1, MaxServiceCurve max_service_curve_2 ) {
		...
		if ( latency_msc_1.equals( Num.getPositiveInfinity() ) ) {
			return max_service_curve_2.copy();
		}
		if ( latency_msc_2.equals( Num.getPositiveInfinity() ) ) {
	...
	public static ServiceCurve convolve_SC_SC_Generic( ServiceCurve service_curve_1, ServiceCurve service_curve_2 ) {
		...
		if ( service_curve_1.isBurstDelay() && service_curve_1.getSegment(1).getX().equals( Num.getZero() ) ) {
			return service_curve_2.copy();
		}
		if ( service_curve_2.isBurstDelay() && service_curve_2.getSegment(1).getX().equals( Num.getZero() ) ) {
			return service_curve_1.copy();
		}
		...
		Num y = Num.add( service_curve_1.f( Num.getZero() ), service_curve_2.f( Num.getZero() ) );
		Num grad = Num.createZero();
		...
		if (i1 > 0 || i2 > 0) {
			x = Num.createZero();
			y = Num.add( service_curve_1.fLimitRight( Num.getZero() ), service_curve_2.fLimitRight( Num.getZero() ) );
			grad = Num.createZero();
	...
	public static Set<Curve> convolve_ACs_MSC( Set<ArrivalCurve> arrival_curves, MaxServiceCurve maximum_service_curve ) throws Exception {
		...
		for ( ArrivalCurve ac : arrival_curves ) {
			result.add( convolve( ac, msc_as_ac ).shiftRight( msc_latency ) );
		}
	...
	public static Set<ArrivalCurve> convolve_ACs_EGamma( Set<ArrivalCurve> arrival_curves, MaxServiceCurve extra_gamma_curve ) throws Exception {
		if( extra_gamma_curve.getLatency().greater( Num.getZero() ) ) {
