...
public class Deconvolution {
	...
	public static ArrivalCurve deconvolve( ArrivalCurve arrival_curve, ServiceCurve service_curve, boolean tb_rl_optimized ) {
		...
		if( service_curve.equals( ServiceCurve.createNullService() )
				|| service_curve.getLatency().equals( Num.getPositiveInfinity() )
				|| ( service_curve.getSustainedRate().equals( Num.getZero() ) && service_curve.getSegment( service_curve.getSegmentCount() - 1 ).getY().equals( Num.getZero() ) ) ) {
	...
	public static ArrivalCurve deconvolveTB_RL( ArrivalCurve arrival_curve, ServiceCurve service_curve ) {
		...
		if( service_curve.equals( ServiceCurve.createNullService() )
				|| service_curve.getLatency().equals( Num.getPositiveInfinity() )
				|| ( service_curve.getSustainedRate().equals( Num.getZero() ) && service_curve.getSegment( 1 ).getY().equals( Num.getZero() ) ) ) {
			return ArrivalCurve.createNullArrival();
		}
	...
	private static ArrivalCurve deconvolveGeneric( Curve curve_1, Curve curve_2 ) {
		...
		if( curve_2.equals( ServiceCurve.createNullService() )
				|| curve_2.getLatency().equals( Num.getPositiveInfinity() )
				|| ( curve_2.getSustainedRate().equals( Num.getZero() ) && curve_2.getSegment( 1 ).getY().equals( Num.getZero() ) ) ) {
			return ArrivalCurve.createNullArrival();
		}
		...
		for( int i = 1; i < curve_2.getSegmentCount(); i++ ) { // Start at 1 to skip the arrival curve itself (see above):
			...
			if( y_beta.doubleValue() != 0.0 ) { // Need to lower the rest of the result candidate by y. 
				for( int j = 0; j < candidate_tmp.getSegmentCount(); j++ ) {
					...
					candidate_tmp.getSegment(j).setY( Num.sub( y_alpha, y_beta ) );
				}
			}
		...
		for( int i = curve_1.getSegmentCount()-1; i >= 0; i-- ) {
			...
			results_cand_burst = Num.sub( y_alpha, y_beta );
			
			if( x_inflect_alpha.equals( Num.getZero() ) // The inflection point is in the origin and thus the candidate is a null curve.
					|| results_cand_burst.less( Num.getZero() ) ) { // At the inflection point, the service curve is larger than the arrival curve.
					continue;
			}
			...
			for( int j = curve_2.getSegmentCount()-1; j >= 0; j-- ) {
				...
				Num next_x_coord = Num.createZero();
				...
				next_x_coord = Num.sub( x_inflect_alpha, x_inflect_beta );
				next_y_coord = Num.add( results_cand_burst, Num.mult( next_x_coord, current_beta_segment.getGrad() ) );
				...
				for( int k = 2; k < segment_count-1; k++ ) { // < segment_count-1 because the count is max{index}+1 and the last segment will be a horizontal line (added after this loop).
					...
					current_segment_length = Num.sub( prev_beta_segment.getX(), current_beta_segment.getX() );
					next_x_coord = Num.add( next_x_coord, current_segment_length ); // Prev > current because we iterate j in decreasing order.
					next_y_coord = Num.add( current_candidate_segment.getY(), Num.mult( current_segment_length, current_beta_segment.getGrad() ) );
				}
